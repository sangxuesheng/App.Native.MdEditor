...
const server = http.createServer((req, res) => {
  ...
  if (parsed.pathname === '/api/image/list' && req.method === 'GET') {
    try {
      // 使用共享目录存储图片
      const imagesBaseDir = '/var/apps/App.Native.MdEditor2/shares/shares/mdeditor/images';
  ...
  if (parsed.pathname === '/api/image/delete' && req.method === 'DELETE') {
    ...
    req.on('end', () => {
      ...
      try {
        // 使用共享目录存储图片
        const imagePath = path.join('/var/apps/App.Native.MdEditor2/shares/shares', imageUrl);
  ...
  if (parsed.pathname === '/api/image/upload' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        // 使用共享目录作为图片存储位置
        const imagesDir = path.join('/var/apps/App.Native.MdEditor2/shares/shares/mdeditor/images', year.toString(), month, day);
  ...
  if (parsed.pathname.startsWith('/images/')) {
    try {
      // 使用共享目录提供图片服务
      const imagePath = path.join('/var/apps/App.Native.MdEditor2/shares/shares', parsed.pathname);
